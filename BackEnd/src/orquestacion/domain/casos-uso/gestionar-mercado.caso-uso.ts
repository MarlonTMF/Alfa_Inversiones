  ...
  async upsertTendencias(dto: ActualizarMercadoDto) {
    try {
      ...
      if (existe.length > 0) {
        ...
        return {
          mensaje: 'Datos de mercado actualizados correctamente (Native PG)',
          accion: 'UPDATE',
        };
      } else {
        await this.dataSource.query(
          ...
          [
            dto.zone,
            dto.city,
            dto.avg_price_per_m2,
            dto.annual_appreciation || null,
          ],
        );
        return {
          mensaje:
            'Nuevos datos de mercado inyectados correctamente (Native PG)',
          accion: 'INSERT',
        };
      }
    } catch (error) {

...
async function bootstrap() {
    ...
    try {
        ...
        console.log(`\x1b[32m[SEED] Super Admin creado exitosamente: ${superAdmin.email}\x1b[0m`);
    } catch (e: any) {
        if (e.message.includes('email ya está registrado')) {
            console.log(`\x1b[33m[SEED] El Super Admin ya existe: ${superAdmin.email}\x1b[0m`);
        } else {
            console.error(`\x1b[31m[SEED] Error al crear Super Admin:\x1b[0m`, e.message);
        }
    }

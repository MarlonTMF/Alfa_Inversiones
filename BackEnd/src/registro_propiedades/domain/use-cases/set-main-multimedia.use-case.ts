...
export class SetMainMultimediaUseCase {
    ...
    async execute(multimediaId: string): Promise<void> {
        ...
        if (target.type !== 'photo') {
            throw new BadRequestException('Solo las imágenes pueden ser marcadas como principal');
        }

        // 3. Obtener todos los recursos de la misma propiedad
        const allMultimedia = await this.multimediaRepository.findByPropertyId(target.propertyId);

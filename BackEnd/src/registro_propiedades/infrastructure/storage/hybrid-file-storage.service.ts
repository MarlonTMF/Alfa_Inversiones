...
export class HybridFileStorageService extends FileStorageService {
    ...
    async uploadFile(file: Buffer, fileName: string, type: 'photo' | 'video'): Promise<FileUploadResult> {
        if (type === 'video') {
    ...
    private async uploadToCloudinary(file: Buffer, fileName: string): Promise<FileUploadResult> {
        return new Promise((resolve, reject) => {
            const uploadStream = cloudinary.uploader.upload_stream(
                { folder: '365_properties/videos', resource_type: 'video' },
                (error, result) => {
                    if (error) return reject(new Error(`Error en Cloudinary: ${error.message}`));
                    resolve({
